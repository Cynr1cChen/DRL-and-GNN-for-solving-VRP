...
import time
from VRP.state_update import update_mask, update_state
...
class GatConv(MessagePassing):
    ...
    def message(self, edge_index_i, x_i, x_j, size_i, edge_attr):
        # 如果 size_i 是整数，把它转换成 PyTorch 张量
        if isinstance(size_i, int):  # 判断 size_i 是否是整数
            size_i = torch.tensor(size_i, device=edge_index_i.device)  # 转换为张量

        x = torch.cat([x_i, x_j, edge_attr], dim=-1)  # 合并特征
        alpha = self.attn(x)  # 计算注意力得分
        alpha = F.leaky_relu(alpha, self.negative_slope)  # 激活函数
        alpha = softmax(alpha, edge_index_i, num_nodes=size_i)  # 修正后的 softmax 调用
